...
def make_map(seed=None):
    ...
    Fleet of spaceshps crashing into crowd of cthulhus.
    """
    rows = ROWS
    cols = COLS
    ...
    url = f"http://localhost:8000/binary_life.html?s1={s1}&s2={s2}"
    url2 = f"http://localhost:8000/binary_life.html?s1={s2}&s2={s1}"
    #url = f"?s1={s1}"
    #url = f"?s2={s2}"
    print("first, try this one:\n")
    print(url)
    print("\nthen, try this one:\n")
    print(url2)
